...
public class NetworkManager {
    ...
    public <MSG> void sendToAll(MSG message) {
        this.retrieveChannel(simpleChannel -> simpleChannel.send(PacketDistributor.ALL.noArg(), message));
    }
    ...
    private void retrieveChannel(Consumer<SimpleChannel> consumer) {
        if (this.channel == null) {
            LogUtil.error("Network channel is not registered. Please register the channel before using it.");
            return;
        }
